  ...
  jsdom.jQueryify(window, 'http://code.jquery.com/jquery-1.4.2.min.js', function (window, jquery) {
    ...
    if (phones.length) {
      ...
      phone.connectivity = {
        ...
        infrared:  boolean(c1.find('table:nth-child(4) th:contains("Infrared")+td images').attr('src')),
        gps:  boolean(c1.find('table:nth-child(4) th:contains("GPS")+td images').attr('src'))
      };
      phone.android = {
      ...
      phone.display = {
        ...
        touchScreen:  boolean(c2.find('table:nth-child(3) th:contains("Touch screen")+td images').attr('src'))
      };
      phone.hardware = {
        fmRadio:  boolean(c2.find('table:nth-child(4) th:contains("FM Radio")+td images').attr('src')),
        physicalKeyboard: c2.find('table:nth-child(4) th:contains("Physical keyboard")+td images').attr('src'),
        accelerometer: boolean(c2.find('table:nth-child(4) th:contains("Accelerometer")+td images').attr('src')),
      ...
      phone.images = [];
      body.find('#thumbs images').each(function(){
